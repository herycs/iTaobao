...
(
   ...
   userID               int,
   pro_ID               int,
   favourite_status     char(1) comment '1表示收藏 0表示未收藏',
...
(
   ...
   pro_ID               int,
   fav_name             varchar(10),
   fav_price            float(2) comment '百分比：原价的多少 整数：抵消一定金额',
   fav_method           int comment '3：优惠券 2：津贴 1：红包 ',
   primary key (fav_ID)
);
...
(
   ...
   createTime           datetime,
   payTime              datetime,
   orderStatus          char(1) comment '3表示已收货 2表示已发货 1表示成功 0表示未付款 -1表示失败  -2表示已退货 ',
   primary key (orderID)
);
...
(
   ...
   pro_status           char(1) comment '2在售 1待售 0预购 -1缺货 -2下架',
   primary key (pro_ID)
);
...
alter table collect add constraint FK_Relationship_17 foreign key (pro_ID)
      references product (pro_ID) on delete restrict on update restrict;
